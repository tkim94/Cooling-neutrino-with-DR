...
# Total neutrino mass   [10^-5, 10] array
Mass_nu_arr = np.logspace(-10,-6,num=13)
...
for ii in range(len(Mass_nu_arr)):
    for jj in range(len(Mass_phi_arr)):
        ...
        with open(prefix_loc+f'mN{int(mN1)}_NH/Temperature_evolution_mN_{int(mN1)}_{int(file_no)}.pkl', 'wb') as f:
